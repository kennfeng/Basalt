...
def test_bulk_ingest_resume_after_partial(tmp_path: Path) -> None:
    from scripts.bulk_ingest import bulk_ingest
    ...
    if checkpoint.exists():
        offset = int(checkpoint.read_text(encoding="utf-8").strip() or "0")
        assert offset == 0
    else:
        assert True
    ...
    assert count == 4
    assert mock_ingestor2.collection.upsert.call_count == 2
